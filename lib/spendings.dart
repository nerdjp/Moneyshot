...
class Spendings extends Data {
	...
	Spendings(this.date, this.description, this.value, this.category, this.datePayment, this.installment, this.totalInstallment) {
		isPaid = datePayment != null ? true : false;
		table = 'spendings';
		//AppDatabase.instance.create(this);
	}

	Spendings.withInstallments(this.date, this.description, this.value, this.category, this.installment, this.totalInstallment);
					...
					Row(
						...
						children: [
							...
							Padding(
								padding: const EdgeInsets.all(16.0),
								child: OutlinedButton(
									child: Text(paymentDate == null ? "Data Pagamento (Opcional)" : DateFormat.yMd().format(paymentDate ?? date)),
									onPressed: () {
										showDatePicker(
											context: context,
											initialDate: DateTime.now(),
